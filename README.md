...
| | 균일 24° (J) | 3.01% | 24° |
| | 밴드2 (J에 블렌드 비용) | **1.24%** | **14.8°** ← 두 축 모두 우세 |
...
서포트 2.4배 적고 왜곡도 38% 적은 해를 고른다. `k_blend` 기본값 0.5의 근거는
`analyze_blend_k.py` 의 '판단이 뒤집히는 창'(측정값 0.1~1.0) 분석이다.
...
- `BLEND_COST_K`       : J의 블렌드 비용 가중치 (기본 0.5, 창 0.1~1.0)
