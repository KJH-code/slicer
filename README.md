...
구간별은 '각도 예산'을 오버행 심한 구간에만 몰아써서 **더 적은 왜곡으로 비슷한
오버행**을 낸다 (서포트를 더 줄이는 것이 아니다 — 아래 표의 경고 참조). `compare_complexity.py`가 균일/구간2/구간3/세밀을 **서포트·강도proxy·
평균각·계산시간**으로 비교한다(복잡도 vs 성능 가성비 곡선).

**단, 툴패스로 실측해 보니 조건부다** (`compare_waist.py`):

> ⚠ **아래 '페리미터 미지지' 열은 오버행이 아닌 몫을 섞어 센다.**
> 위로 좁아지는 형상은 페리미터가 아랫층 희소 인필 위에 놓이는데(평범한 브리징)
> 검사기가 그걸 미지지로 셌다. 허상 비율이 행마다 28~67% 로 달라 **순위를 바꾼다.**
> 갈라낸 수치가 '진짜 오버행' 열이다 — 결론은 이쪽으로 읽어야 한다.
> 자세한 것은 [`docs/verification.md`](docs/verification.md) 2026-09-19 절.

| 모델 | 전략 | 페리미터 미지지 | **진짜 오버행** | 평균 왜곡각 |
|---|---|---|---|---|
| 구 (허리 없음) | 평면 0° | 7.51% | 4.90%p | 0° |
| | 균일 28° (J) | 1.08% | **0.51%p** | 28° |
| | 밴드 (J에 블렌드 비용) | 1.08% | 0.51%p | 28° ← 균일로 수렴 |
| 램프 (허리 있음) | 평면 0° | 4.49% | 2.71%p | 0° |
| | 균일 24° (J) | 3.01% | **1.01%p** | 24° |
| | 밴드2 (J에 블렌드 비용) | 1.78% | 1.12%p | 16.3° |
| | 밴드4 (J에 블렌드 비용) | 1.57% | 1.13%p | **14.8°** |

읽는 법: **원뿔이 평면을 크게 이긴다**(구 4.90→0.51%p, 램프 2.71→1.01%p).
그런데 **밴드는 균일각을 오버행에서 이기지 않는다** — 거의 같은 오버행(1.13 vs
1.01%p)을 **왜곡 38% 적게**(14.8° vs 24.0°) 달성한다. 파레토 우세가 아니라
트레이드오프이고, 어디에 설지는 J 의 각도 비용 k 가 정한다.
...
(회귀 테스트로 강제). 구는 저절로 균일로 수렴하고, 램프에서는 균일각과 거의 같은
오버행을 왜곡 38% 적게 내는 해를 고른다(N=4, 14.8° vs 24.0°). `k_blend` 기본값 0.5의 근거는
