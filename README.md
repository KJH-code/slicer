...
**❌ 결함 아님 — 지지 창 상수 `1.5` 가 `MAX_SPACING_FACTOR` 와 같은 것.**
   가장 의심스러워 보였지만, 창을 1.2~1.8 로 바꿔도 **판정이 안 바뀐다.**
   공유는 사실이나 결과를 만들지는 않았다.

**⚠ 미해결(결함이 아니라 모델링 선택) — 미지지 재료가 지지로 세어진다.**
   `check_support` 는 이전에 퇴적된 **모든** 점을 후보로 쓰고 `supported` 로
   거르지 않는다. `require_supported_below=True` 로 끊으면 수치가 **5~20 배**
   뛴다. 어느 쪽도 옳지 않다 — 현재값은 **낙관적 하한**(처진 비드도 온전한
   지지로 침), 연쇄는 **비관적 상한**(전혀 못 받친다고 봄)이다. 진실은 사이이고,
   **그 사이가 이만큼 넓다는 것**이 지금 아는 전부다. 기본값은 안 바꿨다.
...
|---|---|---|---|---|---|---|---|
| 오버행 감소 | ×1.0 | ×0.9 | ×0.9 | **×1.4** | **×3.3** | **×3.1** | **×4.1** |
