...
> ```
>
> **ρ 는 12 개 케이스에서 예외 없이 승패를 가른다** (승 ≤ 5.40 < 8.14 ≤ 패).
> 둘 다 밴드 문제를 풀기 **전에** 해석식으로 나오므로 사후 설명이 아니라 예측기다.
> ⚠ 문턱은 **1 이 아니다**(그렇게 예상했었다) — (5.40, 8.14) 구간까지만 좁혔다.
> ⚠ ρ 가 `cost` **단독**보다 나은지는 이 표본으로 못 가린다(둘 다 분리).
>
> ```
> python3 analyze_blend_ratio.py --measure     # 위 표 재현 + 분리 검정
> ```
