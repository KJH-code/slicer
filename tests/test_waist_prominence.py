"""T11: 허리 두드러짐 지표가 '밴드가 이기는 조건'을 실제로 가려내는가.

밴드(부위별 각도)가 균일 원뿔을 이기는 조건은 허리의 존재다 — 각도 변경에
필요한 블렌드 폭이 그 높이의 반경에 비례하기 때문이다(docs/verification.md).
`waist_prominence` 는 그 조건을 표본마다 재현 가능한 수로 만든 것이므로,
지표가 조건을 잘못 가리면 표본을 늘린 결과 전체가 무의미해진다.
