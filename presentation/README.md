...
| 9 | 설계에서 우리 코드가 읽어야 할 것 (부품 **3종** 3D 뷰어) | 지금 |
| 10 | 기계가 정해지자 코드가 달라졌다 — **B/C 자세 실시간 애니메이션** | 지금 |
| 11 | 지금 걸려 있는 것 | 지금 |
...
### HTML로 만든 이유 — 내용이 움직인다

정적 슬라이드로는 안 되는 것을 쓰려고 HTML로 만들었습니다.
**장식이 아니라 내용이 움직이는 것**이 먼저입니다 (`rep5x-live.js`).

| 무엇이 움직이나 | 어디 | 왜 |
|---|---|---|
| 헤더의 **해결 · 지금 · 다음** 표시기 | 3~12쪽 | 발표의 어느 토막인지 항상 보인다 |
| 문제 → 답이 오른쪽에서 밀려 들어오고 칩이 찍힌다 | 2쪽 | 대조가 눈에 들어온다 |
| 숫자가 0에서 **세어 올라간다** | 3·8쪽 | "우리가 얻은 숫자"라는 말과 화면이 같아진다 |
| 01→07 로 **신호가 지나가며** 각 단계가 켜진다 | 5쪽 | 파이프라인은 흐름이다 |
| 화면 재현이 실제로 **열린다**(주소창 로딩) | 7쪽 | 재현이라는 것이 분명해진다 |
| 부품 모델이 손대기 전까지 **천천히 돈다** | 9쪽 | 형상을 저절로 보여준다 |
| 부품 셋이 각각 **코드에 무엇을 정해주는지** 로 적혀 있다 | 9쪽 | 출력 목록으로 읽히지 않게 |
| B/C 자세를 **실시간 계산해** 두 시점으로 그린다 | 10쪽 | 코드가 만드는 자세를 눈으로 본다 |
| 아직 안 풀린 카드의 테두리가 **맥동한다** | 11쪽 | 미해결이라는 상태가 보인다 |

그 위에 배경 질감을 얹었습니다 (`rep5x-fx.js`). 전부 **마우스와 무관하게**
스스로 움직입니다 — 커서를 따라다니는 효과는 넣지 않았습니다.

| 무엇 | 어디 | 구현 |
|---|---|---|
| Galaxy 별밭 | 1쪽 배경 | 미분회전하는 별밭 |
| Fuzzy Text | 1쪽 제목 | 캔버스에 그린 뒤 줄마다 흔든다 |
| ASCII Text | 6쪽 배경 워터마크 | 5×7 비트맵 글꼴 + 물결 |
| Ballpit | 13쪽 배경 | 스스로 떠다니며 부딪히는 공 |
...
rep5x-live.js       # 내용이 움직이는 것들 (숫자·그래프·파이프라인·단계 표시기)
rep5x-fx.js         # 배경 질감 (Galaxy·Ballpit·Fuzzy Text·ASCII Text)
assets/models/rep5x/*.stl    # Rep5x 부품 3D 모델 (3MF→STL, NOTICE.md 참고)
assets/images/rep5x/*.jpg    # Rep5x 조립 사진 (축소본)
```

9쪽은 **출력할 부품 목록이 아닙니다.** 공개된 설계에서 우리 코드가 값을 읽어야
하는 지점 셋만 골랐습니다 — 충돌 검사의 대상(B암), C 누적의 근거(슬립링 홀더),
기종 의존이 격리된 곳(캐리지 마운트). 각 부품 설명이 "이 부품이 우리 코드에
무엇을 정해주는가"로 쓰여 있으니 발표에서도 그렇게 말하면 됩니다.

`assets/models/rep5x/` 에는 나머지 7종 STL 도 그대로 있습니다. 보여줄 부품을
바꾸려면 `rep5x-parts.js` 의 `REP5X_PARTS` 객체와 `index.html` 의 `.parts-list`
버튼만 손보면 됩니다.
