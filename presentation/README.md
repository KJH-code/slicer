...
| 파일 | 무엇 | 장수 |
|---|---|---|
| `index.html` | **2026 R&E 중간 성과공유회** (현재 발표) — 검증 체계·다중 모델 검증·통합 파이프라인, 그리고 이번 핵심인 **Rep5x 기반 5축 프린터 설계** | 13 |
...
|---|---|
| 1 | 표지 |
| 2 | 지난 발표가 남긴 문제와 그 뒤 해결한 것 |
| 3 | 검증용 코드 설계 — 5개 검증 계층과 각 층이 보증 못 하는 것 |
| 4 | 여러 3D 모델에 대한 원뿔각 설정 검증 (구·램프·funnel) |
| 5 | 통합 파이프라인 설계 완료 |
| 6 | 이번 핵심이 3D 프린터 설계인 이유 |
| 7 | Rep5x 공개 홈페이지 화면 재현 (링크 없음) |
| 8 | 실물 사진과 5축 설계 구성 |
| 9 | 부품별 설명 + **3D 모델 전환 뷰어** (부품 10종) |
| 10 | 실제 프린터 모델 기반 코드 설계 — **B/C 자세 실시간 애니메이션** + 영상 자리(비어 있음) |
| 11 | 현재까지의 문제점 |
| 12 | 앞으로의 목표 |
| 13 | 출처 |
...
- **10쪽 영상 자리** — 조립·캘리브레이션 후 촬영한 5축 출력 영상을
  `.code-video` 안의 `.video-placeholder` 자리에 `<video>` 로 교체합니다.
  (지난 발표의 `2026-07-27-midterm.html` 9쪽에 `<video data-slide-autoplay>` 사용 예가 있습니다.)
...
```text
index.html          # 13개 슬라이드
rep5x.css           # 이 발표에서 새로 생긴 레이아웃 (styles.css 위에 얹힘)
rep5x-deck.js       # 발표자 메모 (presentation.js 보다 먼저 로드)
rep5x-parts.js      # 9쪽 부품 탐색기 — 부품 데이터와 3D 뷰어 연결
rep5x-motion.js     # 슬라이드 진입 연출 + 10쪽 B/C 자세 애니메이션
assets/models/rep5x/*.stl    # Rep5x 부품 3D 모델 (3MF→STL 변환, NOTICE.md 참고)
assets/images/rep5x/*.jpg    # Rep5x 조립 사진 (축소본)
```

### HTML로 만든 이유

정적 슬라이드로는 안 되는 것들을 쓰려고 HTML로 만들었습니다.

- **9쪽** — 출력할 부품 10종을 실제 3D 모델로 돌려 보며 설명합니다(Three.js).
- **10쪽** — 원뿔 레이어를 도는 압출점에서 B(틸트)·C(요)가 어떻게 정해지는지를
  **코드와 같은 식으로 실시간 계산해** 그립니다. 왼쪽 위에 B·C·θ(z) 값이,
  아래에 그 순간의 G-code 한 줄이 같이 나옵니다. `일시정지` 버튼으로 한 자세에서
  멈춰 설명할 수 있습니다. (`rep5x-motion.js` 의 `ConicalNozzleScene`)
- **7쪽** — Rep5x 홈페이지를 링크 없이 화면으로 재현했습니다(오프라인 발표 대비).
- **전 슬라이드** — 진입할 때 카드가 읽는 순서대로 올라옵니다. 파이프라인(5쪽)처럼
  순서가 의미인 곳에서 특히 도움이 됩니다.

`prefers-reduced-motion` 을 켠 환경에서는 움직임이 자동으로 꺼집니다.

부품 설명 문구와 사진은 전부 `rep5x-parts.js` 상단의 `REP5X_PARTS` 객체 한 곳에
...
> [`assets/models/rep5x/NOTICE.md`](assets/models/rep5x/NOTICE.md) 에 적어 두었습니다.
...
- **10쪽 실제 출력 영상이 아직 없습니다.** 조립·캘리브레이션 후 촬영해 채워야 합니다.
- **9쪽 3D 모델은 HTTP 서버에서만 자동 로드됩니다.** `file://` 로 열면 안내 문구가 표시됩니다.
- 10쪽의 `conical/rep5x.py` 코드는 **설계안이며 아직 구현·검증 전**입니다(11쪽 02번 문제).
