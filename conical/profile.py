...
⚠ 검사기가 실측으로 찾은 추가 제약 (층간격 팽창 — 안정 방향에도 해당):
    연속 레이어의 수직 간격은 반경 r에서 Δz = ΔZw·(1 − c·r·s). 각도가 '감소'하는
    안정 방향(c·s<0)은 가역성 제약이 없지만 간격이 (1+r·|s|)배로 '팽창'한다 —
    구의 적도(r=10)에서 0.5mm 블렌드로 36°→0° 전환 시 레이어가 수직 ~6mm씩
    벌어져 툴패스 검사기 미지지 25%가 실측됨(compare_prediction_vs_toolpath).
    간격을 1.5×층고 이내로 유지하려면 w ≥ 2·r_blend·|Δtan| 이 필요하다.
    현재 from_bands 는 지시서 스펙(가역성 w_min)만 강제한다 — 층간격 제약은
    전이 위치 최적화(반경 작은 높이에 경계 배치)와 함께 향후 과제.
"""
